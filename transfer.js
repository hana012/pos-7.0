    ...
    newRow.innerHTML = `
        ...
        <td>
            <div style="position: relative; width: 100%;">
                <input type="text" id="item-name-${rowCount}" placeholder="Enter item name" onclick="event.stopPropagation()" ondblclick="openItemQuickSearch(${rowCount})" autocomplete="off" style="width: 100%; padding-right: 30px;">
...
function loadTransfer(index) {
    ...
    document.getElementById('payment-method').value = transfer.paymentMethod || 'cash';
...
function showItemSuggestions(query, rowNumber) {
    ...
    if (!suggestionsContainer) {
        ...
        container.style.cssText = 'position: absolute; top: 100%; left: 0; right: 0; background: white; border: 1px solid #ddd; border-radius: 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); z-index: 1000; max-height: 400px; overflow-y: auto; display: none;';
    ...
    if (!query || query.trim().length === 0) {
        if (items.length === 0) {
            suggestionsContainer.innerHTML = '<div style="padding: 12px; color: #6b7280; font-style: italic;">No items found. Add items in the Items page first.</div>';
        ...
        suggestionsContainer.innerHTML = displayItems.map(item => {
            ...
            return `
                <div class="item-suggestion" onclick="selectItemSuggestion(${rowNumber}, '${itemName.replace(/'/g, "\\'")}', '${barcode.replace(/'/g, "\\'")}')" style="padding: 12px; cursor: pointer; border-bottom: 1px solid #eee; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'" onmouseout="this.style.backgroundColor='white'">
                    <strong style="color: #1f2937;">${itemName}</strong>
                    ${barcode ? `<br><small style="color: #6b7280;">Barcode: ${barcode}</small>` : ''}
                </div>
            `;
        }).join('');
        
        if (items.length > 20) {
            suggestionsContainer.innerHTML += `<div style="padding: 12px; color: #6b7280; font-style: italic; text-align: center; background: #f9fafb;">Showing 20 of ${items.length} items. Type to filter...</div>`;
        }
    ...
    const filtered = items.filter(item => {
        ...
        return itemName.includes(lowerQuery) || barcode.includes(lowerQuery);
    }).slice(0, 15);
    
    if (filtered.length === 0) {
        suggestionsContainer.innerHTML = '<div style="padding: 12px; color: #6b7280; font-style: italic;">No matching items found. Try a different search term.</div>';
        suggestionsContainer.style.display = 'block';
        return;
    }
