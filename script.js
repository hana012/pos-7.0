...
function initializeInvoice() {
    ...
    document.getElementById('invoice-time').textContent = timeString;
...
function addEventListenersToRow(rowNumber) {
    ...
    if (itemNameInput) {
        // Show all items when field is focused
        itemNameInput.addEventListener('focus', function(event) {
            const query = event.target.value.trim();
    ...
    newRow.innerHTML = `
        ...
        <td>
            <div style="position: relative; width: 100%;">
                <input type="text" id="item-name-${rowId}" placeholder="Enter item name" onclick="event.stopPropagation()" ondblclick="openItemQuickSearch(${rowId})" autocomplete="off" style="width: 100%; padding-right: 30px;">
                <i class="fas fa-search" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); color: #6b7280; pointer-events: none; font-size: 14px;"></i>
            </div>
        </td>
...
function loadInvoice(index, direction = 'next') {
    ...
    setTimeout(() => {
        try {
            ...
            if (paymentMethodEl) paymentMethodEl.value = invoice.paymentMethod || 'cash';
...
async function saveInvoiceWithCustomerIntegration() {
    ...
    try {
        if (typeof window !== 'undefined' && window.LoadingManager) {
            window.LoadingManager.showOverlay('Saving invoice...', false);
        }
    } catch (error) {
    ...
    // Record inventory activities for each item sold
    recordInventoryActivities(invoiceData);
    
    // Update inventory stock levels for sold items
    if (typeof window.updateInventoryStockOnSale === 'function') {
        invoiceData.items.forEach(item => {
            if (item.quantity > 0 && item.itemName) {
                window.updateInventoryStockOnSale(item.itemName, item.quantity);
            }
        });
    }
...
function showItemSuggestions(query, rowNumber) {
    ...
    if (!suggestionsContainer) {
        ...
        const inputRect = itemNameInput.getBoundingClientRect();
    ...
    // Update position each time (in case input moved or page scrolled)
    const itemNameInput = document.getElementById(`item-name-${rowNumber}`);
    if (!itemNameInput) return;
    const inputRect = itemNameInput.getBoundingClientRect();
    suggestionsContainer.style.cssText = `
    ...
    if (!query || query.trim().length === 0) {
        if (allItems.length === 0) {
            suggestionsContainer.innerHTML = `
                <div class="item-suggestion" style="color: #6b7280; font-style: italic;">
        ...
        const displayItems = sortedItems.slice(0, 20); // Show first 20 items
        
        suggestionsContainer.innerHTML = displayItems.map(item => {
            const isService = item.itemType === 'service';
            const serviceIcon = isService ? '🔧 ' : '';
            ...
            return `
                <div class="item-suggestion" onclick="selectItemFromSuggestion('${item.name.replace(/'/g, "\\'")}', '${item.barcode.replace(/'/g, "\\'")}', '${item.retailPrice}', ${rowNumber})">
                    <div class="item-suggestion-name">${serviceIcon}${item.name.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
                    <div class="item-suggestion-details">Barcode: ${item.barcode.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')} | ${stockInfo} | ${item.category || 'No category'}</div>
                    <div class="item-suggestion-price">${priceText}</div>
                </div>
            `;
        }).join('');
        ...
        if (allItems.length > 20) {
            suggestionsContainer.innerHTML += `
                <div class="item-suggestion" style="color: #6b7280; font-style: italic; text-align: center;">
                    Showing 20 of ${allItems.length} items. Type to filter...
                </div>
            `;
        }
        ...
        suggestionsContainer.style.display = 'block';
        // Update position
        const inputRectUpdate = itemNameInput.getBoundingClientRect();
        suggestionsContainer.style.top = `${inputRectUpdate.bottom + window.scrollY}px`;
        suggestionsContainer.style.left = `${inputRectUpdate.left + window.scrollX}px`;
        suggestionsContainer.style.width = `${inputRectUpdate.width}px`;
        return;
    }
    ...
    if (itemsWithScores.length === 0) {
            ...
            suggestionsContainer.style.display = 'block';
        // Update position
        const inputRectUpdate2 = itemNameInput.getBoundingClientRect();
        suggestionsContainer.style.top = `${inputRectUpdate2.bottom + window.scrollY}px`;
        suggestionsContainer.style.left = `${inputRectUpdate2.left + window.scrollX}px`;
        suggestionsContainer.style.width = `${inputRectUpdate2.width}px`;
        return;
    }
    
    // Show top 15 most relevant results
    const topItems = itemsWithScores.slice(0, 15).map(result => result.item);
    
    // Update position before showing
    const inputRectUpdate3 = itemNameInput.getBoundingClientRect();
    suggestionsContainer.style.top = `${inputRectUpdate3.bottom + window.scrollY}px`;
    suggestionsContainer.style.left = `${inputRectUpdate3.left + window.scrollX}px`;
    suggestionsContainer.style.width = `${inputRectUpdate3.width}px`;
    ...
    suggestionsContainer.innerHTML = topItems.map(item => {
        ...
        return `
            <div class="item-suggestion" onclick="selectItemFromSuggestion('${item.name.replace(/'/g, "\\'")}', '${item.barcode.replace(/'/g, "\\'")}', '${item.retailPrice}', ${rowNumber})">
                <div class="item-suggestion-name">${serviceIcon}${highlightedName}</div>
                <div class="item-suggestion-details">Barcode: ${highlightedBarcode} | ${stockInfo} | ${item.category || 'No category'}</div>
                <div class="item-suggestion-price">${priceText}</div>
            </div>
        `;
    }).join('');
